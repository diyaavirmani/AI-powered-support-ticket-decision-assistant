...
| DB-01 | Mandatory | Persist users, tickets, and decisions in SQLite with relationships and timestamps. | SQLAlchemy 2.x models/session | Temporary-database integration tests | Foundation complete; ticket persistence pending endpoint |
| DB-02 | Mandatory | Persist `order_value_inr`, `days_since_delivery`, `days_since_dispatch`, `product_type`, `opened_status`, and `order_status`. | Extended `tickets` table | Schema inspection and API round-trip tests | Schema complete; round-trip pending endpoint |
| DB-03 | Mandatory | Persist structured decision action, reason, confidence, sources, and inferred issue type for reproducibility/audit. | `decisions` table | Database and detail-response tests | Schema complete; decision pipeline pending |
...
| SCM-01 | Mandatory | Publish a public GitHub repository with genuine milestone commits on default branch `main`. | Git/GitHub | Remote inspection, push, remote log, clean status | Public repository verified; local milestones pending authentication for push |
