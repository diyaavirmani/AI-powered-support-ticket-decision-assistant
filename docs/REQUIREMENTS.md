...
| RAG-03 | Mandatory | Infer issue type from ticket content because evaluation inputs omit it. | Structured Gemini decision schema and prompt | No caller issue type; evaluation input contract validation | Complete; evaluation execution pending |
...
| TEST-01 | Mandatory | Test important API, authentication, authorization, retrieval, AI validation, persistence, and failure behavior. | `tests/` with pytest/FastAPI client | Full pytest run and coverage review | Phase 3 backend slice complete; UI/evaluation tests pending |
| DOC-01 | Mandatory | Provide setup, usage, architecture, trade-offs, limitations, and secret-handling documentation. | README, architecture, development log | Documentation review from clean checkout | In progress; frontend/evaluation instructions pending |
...
| SEC-01 | Mandatory | Keep API keys, JWT secrets, `.env`, databases, caches, and runtime files out of Git. | Settings, `.env.example`, `.gitignore` | `git check-ignore` and staged-file scan | Complete; rechecked for Phase 3 |
| SCM-01 | Mandatory | Publish a public GitHub repository with genuine milestone commits on default branch `main`. | Git/GitHub | Remote inspection, push, remote log, clean status | Public remote already exists; Phase 3 commits await operator's manual push |
